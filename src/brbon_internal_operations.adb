...
package body BRBON_Internal_Operations is
   ...
   function Next (Source: in out Crc_Unbounded_String; Byte: out Unsigned_8) return Boolean is
   begin
      if Source.Remainder = 0 then return False; end if;
      Source.Remainder := Source.Remainder - 1;
      Byte := Character'Pos(Element(Source.Str, (Length (Source.Str) - Natural(Source.Remainder))));
      return True;
   end Next;
   ...
   function Crc_16 (Source: in out Crc_Data'Class; Initalization: Unsigned_16 := 0; Polynomial: Unsigned_16 := 16#A001#) return Unsigned_16 is
   ...
   function Crc_16 (Str: String) return Unsigned_16 is
      UStr: Unbounded_String := To_Unbounded_String (Str);
      Source: Crc_Unbounded_String := (Unsigned_32 (Length (UStr)), UStr);
   begin
      return Crc_16 (Source);
   end Crc_16;


   function Crc_32 (Str: String) return Unsigned_32 is
      UStr: Unbounded_String := To_Unbounded_String (Str);
      Source: Crc_Unbounded_String := (Unsigned_32 (Length (UStr)), UStr);
   begin
