...
function Create_Blocks_With_Color (Count: in out Integer) return Test_Result is
...
begin
   ...
   Expected_Bytes.all (16#50# .. 16#5F#) :=
     (
      16#16#, 16#00#, 16#00#, 16#00#,  16#10#, 16#00#, 16#00#, 16#00#,
   ...
   if Static_Unprotected.Get_Type (P) /= RGBA_Type then
      New_Line (2);
      Put_Line ("Expected the type 'RGBA_Type', found:" & Static_Unprotected.Get_Type (P)'Image);
      return Failed;
   end if;
   ...
   if Static_Unprotected.Get_Name (P) /= "" then
      New_Line (2);
      Put_Line ("Expected no name, found: " & Static_Unprotected.Get_Name (P));
      return Failed;
   end if;
   ...
   if Static_Unprotected.Get_Color (P) /= Color_Package.None then
      New_Line (2);
      Put_Line ("Expected color none, found:" & Color_Package.Image (Static_Unprotected.Get_Color (P)));
      return Failed;
   end if;

   declare
      A_Color: Color_Package.Color := Color_Package.Factory (Red   => 15,
                                                             Green => 22,
                                                             Blue  => 134,
                                                             Alpha => 55);
   begin
      Static_Unprotected.Set_Color (P, A_Color);

      if Static_Unprotected.Get_Color (P) /= A_Color then
         New_Line (2);
         Put_Line ("Expected value '15 22 134 55', found:" & Color_Package.Image (Static_Unprotected.Get_Color (P)));
         return Failed;
      end if;
   end;
   ...
   Expected_Bytes.all (16#50# .. 16#5F#) :=
     (
      16#16#, 16#00#, 16#00#, 16#00#,  16#10#, 16#00#, 16#00#, 16#00#,
      16#00#, 16#00#, 16#00#, 16#00#,     15,     22,    134,     55
     );
