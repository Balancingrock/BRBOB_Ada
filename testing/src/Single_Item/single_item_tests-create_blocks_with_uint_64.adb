...
function Create_Blocks_With_UInt_64 (Count: in out Integer) return Test_Result is
...
begin
   ...
   Expected_Bytes.all (16#50# .. 16#7F#) :=
     (16#0A#, 16#00#, 16#00#, 16#18#,  16#30#, 16#00#, 16#00#, 16#00#,
   ...
   P := T_Object.Add_Root_Item (Of_Type         => BRBON.Types.UInt_64_Type,
                                With_Byte_Count => 0,
   ...
   if Static_Unprotected.Get_Type (P) /= UInt_64_Type then
      New_Line (2);
      Put_Line ("Expected the type 'UInt_64_Type', found:" & Static_Unprotected.Get_Type (P)'Image);
      return Failed;
   end if;
   ...
   if Static_Unprotected.Get_UInt_64 (P) /= 0 then
      New_Line (2);
      Put_Line ("Expected initial value '0', found:" & Static_Unprotected.Get_UInt_64 (P)'Image);
      return Failed;
   end if;

   Static_Unprotected.Set_UInt_64 (P, 16#1234_5678_9ABC_DEF0#);

   if Static_Unprotected.Get_UInt_64 (P) /= 1311768467463790320 then
      New_Line (2);
      Put_Line ("Expected value '1311768467463790320', found:" & Static_Unprotected.Get_UInt_64 (P)'Image);
      return Failed;
   end if;
...
end Create_Blocks_With_UInt_64;
