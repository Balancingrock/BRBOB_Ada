...
function Create_Blocks_With_UUID (Count: in out Integer) return Test_Result is
...
begin
   ...
   Expected_Bytes.all (16#50# .. 16#77#) :=
     (16#15#, 16#00#, 16#00#, 16#08#,  16#28#, 16#00#, 16#00#, 16#00#,
      16#00#, 16#00#, 16#00#, 16#00#,  16#00#, 16#00#, 16#00#, 16#00#,
      16#36#, 16#7F#, 16#04#, 16#55#,  16#55#, 16#49#, 16#44#, 16#00#,
      16#00#, 16#00#, 16#00#, 16#00#,  16#00#, 16#00#, 16#00#, 16#00#,
      16#00#, 16#00#, 16#00#, 16#00#,  16#00#, 16#00#, 16#00#, 16#00#
     );
   ...
   P := T_Object.Add_Root_Item (Of_Type         => BRBON.Types.UUID_Type,
                                With_Byte_Count => 0,
                                With_Name       => "UUID");
   ...
   if Static_Unprotected.Get_Type (P) /= UUID_Type then
      New_Line (2);
      Put_Line ("Expected the type 'UUID_Type', found:" & Static_Unprotected.Get_Type (P)'Image);
      return Failed;
   end if;
   ...
   if Static_Unprotected.Get_Name (P) /= "UUID" then
      New_Line (2);
      Put_Line ("Expected 'UUID', found: " & Static_Unprotected.Get_Name (P));
      return Failed;
   end if;
   ...
   if Static_Unprotected.Get_Byte_Count (P) /= 40 then
      New_Line (2);
      Put_Line ("Expected Byte_Count of 40, found:" & Static_Unprotected.Get_Byte_Count (P)'Image);
      return Failed;
   end if;

   if Static_Unprotected.Get_Value_Area_Byte_Count (P) /= 16 then
      New_Line (2);
      Put_Line ("Expected Value_Area_Byte_Count of 16, found:" & Static_Unprotected.Get_Value_Area_Byte_Count (P)'Image);
      return Failed;
   end if;

   if Static_Unprotected.Get_UUID (P) /= UUID_Package.Nil_UUID then
      New_Line (2);
      Put_Line ("Expected initial value 'Nil_UUID', found:" & Static_Unprotected.Get_UInt_64 (P)'Image);
      return Failed;
   end if;

   declare
      UUIDBytes: UUID_Package.UUID_Bytes := (11, 12, 13, 14, 15, 16, 17, 18, 41, 42, 43, 44, 45, 46, 47, 48);
      A_UUID: UUID_Package.UUID := UUID_Package.Factory (UUIDBytes);
   begin
      Static_Unprotected.Set_UUID (P, A_UUID);
      if Static_Unprotected.Get_UUID (P) /= A_UUID then
         New_Line (2);
         Put_Line ("Expected value 16#0B0C0D0E0F101112_292A2B2C2D2E2F30#, found:" & Static_Unprotected.Get_UInt_64 (P)'Image);
         return Failed;
      end if;
   end;
