...
function Create_Blocks_With_UInt_32 (Count: in out Integer) return Test_Result is
...
begin
   ...
   Expected_Bytes.all (16#50# .. 16#77#) :=
     (16#09#, 16#00#, 16#00#, 16#18#,  16#28#, 16#00#, 16#00#, 16#00#,
   ...
   P := T_Object.Add_Root_Item (Of_Type         => BRBON.Types.UInt_32_Type,
                                With_Byte_Count => 0,
   ...
   if Static_Unprotected.Get_Type (P) /= UInt_32_Type then
      New_Line (2);
      Put_Line ("Expected the type 'UInt_32_Type', found:" & Static_Unprotected.Get_Type (P)'Image);
      return Failed;
   end if;
   ...
   if Static_Unprotected.Get_UInt_32 (P) /= 0 then
      New_Line (2);
      Put_Line ("Expected initial value '0', found:" & Static_Unprotected.Get_UInt_32 (P)'Image);
      return Failed;
   end if;

   Static_Unprotected.Set_UInt_32 (P, 16#3456789A#);

   if Static_Unprotected.Get_UInt_32 (P) /= 878082202 then
      New_Line (2);
      Put_Line ("Expected value '878082202', found:" & Static_Unprotected.Get_UInt_32 (P)'Image);
      return Failed;
   end if;
...
end Create_Blocks_With_UInt_32;
