...
function Create_Blocks_With_Int_16 (Count: in out Integer) return Test_Result is
...
begin
   ...
   Expected_Bytes.all (16#50# .. 16#77#) :=
     (16#04#, 16#00#, 16#00#, 16#18#,  16#28#, 16#00#, 16#00#, 16#00#,
   ...
   P := T_Object.Add_Root_Item (Of_Type         => BRBON.Types.Int_16_Type,
                                With_Byte_Count => 0,
   ...
   if Static_Unprotected.Get_Type (P) /= Int_16_Type then
      New_Line (2);
      Put_Line ("Expected the type 'Int_16_Type', found:" & Static_Unprotected.Get_Type (P)'Image);
      return Failed;
   end if;
   ...
   if Static_Unprotected.Get_Int_16 (P) /= 0 then
      New_Line (2);
      Put_Line ("Expected initial value '0', found:" & Static_Unprotected.Get_Int_16 (P)'Image);
      return Failed;
   end if;

   Static_Unprotected.Set_Int_16 (P, 16#3456#);

   if Static_Unprotected.Get_Int_16 (P) /= 13398 then
      New_Line (2);
      Put_Line ("Expected value '13398', found:" & Static_Unprotected.Get_Int_16 (P)'Image);
      return Failed;
   end if;
